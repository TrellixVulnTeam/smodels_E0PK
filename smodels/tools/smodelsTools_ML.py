...
def main():
    ...
    ML_xseccomputer = subparsers.add_parser('ML_xseccomputer', description="Compute IDM cross sections from trained Neural Network.")
    ML_xseccomputer.add_argument('-s','--sqrts',nargs='+', action='append',
        help="sqrt(s) TeV. Only 8 and 13 supported. Can supply more than one value (as a space separated list). Default is both 8 and 13.",
        type=int, default=[])
    ML_xseccomputer.add_argument('-m','--model',
        help="Model for which to compute the cross sections. Only IDM supported at present",
