...
class PyhfTest(unittest.TestCase):
    def simpleJson(self, bkg, obs):
        ...
        samples = [dict(name="bkg", data=bkg, modifiers=modifiers)]
        channels = [dict(name="SR1", samples=samples)]
        # Defining the measurements
        config = dict(
            poi="mu_SIG",
            parameters=[
                dict(auxdata=[1], bounds=[[0.915, 1.085]], inits=[1], sigmas=[0.017], name="lumi")
            ],
        )
        measurements = [dict(name="BasicMeasurement", config=config)]
        # Defining the observations
        observations = [dict(name="SR1", data=obs)]
        ws = dict(
            channels=channels, measurements=measurements, observations=observations, version="1.0.0"
        )
        return ws
    ...
    def testCorruptJson1Signal(self):
        ...
        ws = dict(  # channels=channels,
            measurements=measurements, observations=observations, version="1.0.0"
        )
    ...
    def testCorruptJson2Signal(self):
        ...
        modifiers = []
        modifiers.append(dict(data=None, type="lumi", name="lumi"))
        samples = [dict(name="bkg", data=[10, 9], modifiers=modifiers)]
        channels = [dict(name="SR1", samples=samples)]
        # Defining the measurements
        config = dict(
            poi="mu_SIG",
        ...
        measurements = [dict(name="BasicMeasurement", config=config)]
        # Defining the observations
        observations = [dict(name="SR1", data=[0.9, 0.8])]
        # Missing channels
        ws = dict(  # channels=channels,
            measurements=measurements, observations=observations, version="1.0.0"
        )
            ...
            dict(
                ...
                value=dict(
                    ...
                    modifiers=[
                        ...
                        dict(name="mu_SIG", type="normfactor", data=None),
                    ],
                ),
            )
        ]
